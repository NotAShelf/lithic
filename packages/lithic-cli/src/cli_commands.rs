...
pub struct Cli {
   ...
   /// If you use this to work on a custom modpack, you will need to run Lithic modpack create again to update your modpack file, just set the --mpk-id to the same one you used before to overwrite the old one.
   #[arg(short, long)]
...
pub enum Commands {
   ...
   #[command(about = "Manage config options for Lithic")]
   Config(ConfigCommand),

   #[command(
      about = "Miscellaneous items for Lithic, like shell auto-completion and 1-click mod installation"
   )]
   Misc {
      ...
      #[cfg(unix)]
      /// Set up the 1-click mod installation. Installs to ~/.local/share/applications/lithic.desktop
      #[arg(short, long)]
   ...
   #[command(
      name = "self",
      about = "Manage the Lithic binary; Check for updates, perform updates."
   )]
   LithicSelf(UpdaterArgs),
