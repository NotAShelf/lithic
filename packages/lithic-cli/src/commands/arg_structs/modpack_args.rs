...
pub enum ModpackSubCommands {
   /// Create a new mod pack. For a better guide, check the wiki: https://github.com/Tekunogosu/Lithic/wiki/Modpacks
   ///
   /// When you create a new modpack, the pack file itself will be save to ~/.config/lithic/modpacks/mypacks/yourmod.zip
   ///
   /// The mods found in the mod_dir you are creating from, will be MOVED into ~/.config/lithic/modpacks/installed/yourmod.
   ...
   ///
   /// Once your mod has been created, you can use modpack local commands to manage your modpacks, see `Lithic modpack help local`
   ///
   #[command(about = "Create a new modpack")]
   Create(MPCreateArgs),

   /// Installing a modpack is like installing a mod, except it is treated differently with Lithic.
   ///
   /// Lithic will create a new alias for the modpack and a directory to store the mods associated.
   /// This makes it easier to manage multiple modpacks without affecting the entire mod system
   ///
   /// By default, all modpacks are installed to ~/.config/lithic/modpacks/alias_name
   ...
   /// Enabling a modpack starts by moving the existing Mods directory *if not a modpack itself* to ~/.config/lithic/modpacks/backup/date-time/{Mods,ModData,ModConfig}
   ///
...
pub struct MPCreateArgs {
   /// *Required* This is the long form name of your modpack -- Example: "Theysa magic mod pack"
   #[arg(short, long)]
   pub name: String,

   /// *Required* This is the ID lithic will use -- Example: "theysa_magicmpk". This is also the id that should be set as the url on the mods website, so they match
   #[arg(short, long)]
   pub mpk_id: String,
   ...
   /// *Optional* Location to save modpack, default is ~/.config/lithic/modpacks/mypacks
...
pub struct MPInstallArgs {
   /// This is the ID from the mods website. This can either be the numerical ModID or the text version. Use `Lithic search -q modpackname` to get the numerical ID if you need
   pub mod_id: String,
...
pub struct MPEnableArgs {
   /// ID of the modpack to enable. See `Lithic modpack list` to get the correct modpack ID if you don't know it.
   pub mpk_id: String,

   /// --force allows you to enable multiple modpacks at once. NOTE: You may make your world unstable by having modpacks with conflicting mods. There is no way for Lithic to check this, you have been warned.
   #[arg(short, long, default_value = "false")]
   pub force: bool,
}
