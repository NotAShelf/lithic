...
async fn set(args: &CommonArgs) {
   ...
   if let Some(version) = &args.pin_game_version {
      ...
      if game_versions.game_versions.contains(&v) {
         ...
         notice(
            "Invalid game version. The version must be one of the following: ",
            Some(Color::Yellow),
            vec![Attribute::Bold],
         );
         notice(
            format!("[{}]", game_versions.game_versions.join("], [").as_str()),
            Some(Color::Magenta),
            vec![],
         );
      }
   ...
   if let Some(with_mod) = &args.with_mod {
      if let Some(version) = &args.pin_version {
         ...
         notice(
            "Be sure to run the sync command to update Lithic's sync file to use the newly set pinned mod version.",
            Some(Color::Green),
            vec![],
         );
      }
   ...
   if let Some(mpk_id) = &args.modpack_enabled {
      if config.modpacks.enabled.contains(mpk_id) {
         display_vec.push(command_output(
            "This modpack ID is already part of config.modpacks.enabled",
            mpk_id,
         ));
      } else {
   ...
   if let Some(mpk_id) = &args.modpack_disabled {
      if config.modpacks.disabled.contains(mpk_id) {
         display_vec.push(command_output(
            "This modpack ID is already part of config.modpacks.disabled",
            mpk_id,
         ));
      } else {
   ...
   if let Some(check) = args.check_for_updates {
      ...
      display_vec.push(command_output(
         ...
         args.check_for_updates.unwrap_or_default().to_string(),
      ));
   }
...
async fn del(args: &DelArgs) {
   ...
   if args.modpack_dir {
      config
         .modpacks
         .modpack_dir
         .clone_from(&defaults.modpacks.modpack_dir);
      save = true;
      display_vec.push(command_output(
   ...
   if args.backup_mods {
      ...
      display_vec.push(command_output(
         "config.backup_mods",
         defaults.backup_mods.to_string(),
      ));
   }
   ...
   if args.pin_game_version {
      config
         .pinned_game_version
         .clone_from(&defaults.pinned_game_version);
      save = true;
      display_vec.push(command_output(
         "config.pinned_game_version",
         defaults.pinned_game_version,
      ));
   }
   ...
   if args.notify_of_unzipped_mods {
      ...
      save = true;
      display_vec.push(command_output(
         "config.notify_of_unzipped_mods",
         config.notify_of_unzipped_mods.to_string(),
      ));
   }
   ...
   if args.pinned_mod.is_some() {
      let Some(mod_id) = &args.pinned_mod else {
         warn!(
            "You must provide a Mod ID before anything can be removed. Run [./Lithic config list] to show all valid options"
         );
         exit(1);
      };
   ...
   if let Some(mpk_id) = &args.modpack_enabled {
      if config.modpacks.enabled.contains(mpk_id) {
         ...
         display_vec.push(command_output(
            "This modpack id is not in the enabled list",
            mpk_id,
         ));
      }
   }
   ...
   if let Some(mpk_id) = &args.modpack_disabled {
      if config.modpacks.disabled.contains(mpk_id) {
         ...
         display_vec.push(command_output(
            "This modpack id is not in the disabled list",
            mpk_id,
         ));
      }
   }
...
async fn list() {
   ...
   let display_vec: Vec<(CellData, CellData)> = vec![
      ...
      command_output(
         ...
         config.show_execution_time.to_string(),
      ),
      command_output(
         "config.notify_of_unzipped_mods",
         config.notify_of_unzipped_mods.to_string(),
      ...
      command_output(
         ...
         config.update_default_windows_loc.to_string(),
      ),
      command_output("", ""),
      command_output(
         "config.modpacks.modpack_dir",
