use crate::commands::arg_structs::config_table_args::{
   ResetArgs, TableArgs, TableArgsSubCommands, TableGroup, TableSubCommands, TableSubFlags,
};
use clap::ValueEnum;
...
pub async fn config_table(args: &TableArgs) {
   ...
   match commands {
      TableSubCommands::Set(cmd) => match &cmd.subcommand {
      ...
      TableSubCommands::Del(cmd) => match &cmd.subcommand {
         TableArgsSubCommands::List(arg) => {
...
fn fill_vec_from_section(section: &FlattenMap, the_vec: &mut Vec<(CellData, CellData)>) {
   for (name, property) in section.iter() {
      ...
      let c_txt = CellData::new(
         format!("{name}.color"),
         Some(Color::Green),
         vec![],
         Some(CellAlignment::Left),
      );
      let c_val = CellData::new(
         color.to_string(),
         Some(Color::from(color.clone())),
         vec![],
         Some(CellAlignment::Left),
      );
      let a_txt = CellData::new(
         format!("{name}.attribute"),
         Some(Color::Blue),
         vec![],
         Some(CellAlignment::Left),
      );
      let a_val = CellData::new(
         attr.to_string(),
         Some(Color::from(color.clone())),
         vec![Attribute::from(attr.clone())],
         Some(CellAlignment::Left),
      );
...
where
   T: ValueEnum + Clone + Send + Sync + Display + 'static,
{
