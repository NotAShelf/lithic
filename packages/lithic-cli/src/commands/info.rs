...
pub async fn info(args: &ModInfoArgs) -> Result<(), LithicError> {
   ...
   for (_, mod_info) in mod_info {
      ...
      let headers = vec![
         ...
         prep_cell(
            mod_info.name.clone().unwrap_or_default(),
      ...
      if args.show_description {
         ...
         t2.add_row(Row::from(vec![prep_cell(
            &t2_txt,
            Some(CellColor::Yellow),
            None,
            None,
