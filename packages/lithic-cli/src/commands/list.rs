...
use csv::Writer;
use lithic_core::aliases::ModFileName;
...
pub async fn cmd_list(
   ...
   let header_cells: Vec<Cell> = list_columns
      ...
      .filter_map(|(column, properties)| {
         ...
         if (local_mp_call || modpack_call)
            && matches!(ListColumn::from_str(column), Ok(ListColumn::MissingDeps))
         {
   ...
   let sync_data = if local_mp_call {
      ...
      Some(match get_sync_data(mod_dir, false).await {
         Ok(s) => s,
         Err(_) => { sync(mod_dir, false, vec![]).await }?,
      })
   };
   ...
   for (pack_id, v) in &mut enabled_modpacks {
      ...
      let mpath = Path::new(&config.modpacks.modpack_dir)
         .join("installed")
         .join(pack_id);
      if mpath.exists() {
         ...
         let cells: Vec<Cell> = list_columns
            ...
            .filter_map(|(column, properties)| {
               ...
               match <ListColumn as FromStr>::from_str(column) {
                  ...
                  Ok(ListColumn::PinnedVersion) => Some(
                  ...
                  Ok(ListColumn::Filename) => Some(prep_cell(filename.to_string(), color, attr, None, None)),
                  ...
                  Ok(ListColumn::LastUpdateLocal | ListColumn::LastUpdateRemote | ListColumn::HasBackup) => {
                  ...
                  Ok(ListColumn::Changelog) => {
                     ...
                     let out = html_parse(&mut changelog, usize::from(table.width().unwrap_or_default()))
                        .unwrap_or_default();
                  ...
                  Ok(ListColumn::Website) => Some(prep_cell(
                     mod_info.website.clone().unwrap_or_default().as_str(),
                     color,
                     attr,
                     None,
                     None,
...
fn write_record<I, T>(writer: &mut Writer<Box<dyn Write>>, collection: I) -> Result<(), LithicError>
where
   I: IntoIterator<Item = T>,
   T: AsRef<[u8]>,
{
   writer
      .write_record(collection)
