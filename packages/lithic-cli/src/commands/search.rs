...
use lithic_core::api::structs::{ModApi, ModsSearchFile};
use lithic_core::config::manager::{Config, get_config};
use lithic_core::config::structs::SearchColumn;
use lithic_core::consts::FILE_MOD_SEARCH_SYNC;
use lithic_core::errors::LithicError;
use lithic_core::search::SortBy;
use lithic_core::search::{Field, SearchQuery, SortOrder};
use lithic_core::utils::parse_json_file;
...
pub async fn parse_search_file() -> Result<ModsSearchFile, LithicError> {
   ...
   if !file_path.exists() {
      warn!(
         "{}",
         "Running daily sync to build search table, this is normal!".green()
      );
...
pub async fn search(args: &SearchArgs) -> Result<(), LithicError> {
   ...
   if args.field.is_some() && args.query.is_some() {
      query = query.add_field_search(
         args.field.clone().unwrap_or(Field::Summary),
...
pub async fn show_search_table(results: Vec<ModApi>) {
   ...
   let col_cells: Vec<Cell> = search_headers
      .iter()
      .map(|(k, v)| {
   ...
   let b_rows: Vec<Row> = results
      .iter()
      .map(|m| {
         let cells: Vec<Cell> = search_cells
            .iter()
            .map(|(k, v)| {
               let color = v.color.clone();
               let attr = v.attribute.clone();
